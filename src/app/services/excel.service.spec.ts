...
it('C9 RED: Resumen del Mes no debe incluir Diferencia consolidada', async () => {
  ...
  describe('generarExcelPalmar (palmar-ventas PR2)', () => {
    ...
    it('PR2 RED: devuelve base64 xlsx válido con 3 hojas Resumen, Arqueo, Ventas', async () => {
      const result = await service.generarExcelPalmar(record, resumenSemana);
    ...
    it('PR2 RED: hoja Ventas excluye productos con cantidad 0', async () => {
      const result = await service.generarExcelPalmar(record, resumenSemana);
      const workbook = XLSX.read(result, { type: 'base64' });
      const sheet = workbook.Sheets['Ventas'];
      const json = XLSX.utils.sheet_to_json(sheet, { header: 1 }) as unknown[][];
    ...
    it('PR2 RED: hoja Ventas muestra valores de fila (nombre, cantidad, subtotal, costo, invertido)', async () => {
      const result = await service.generarExcelPalmar(record, resumenSemana);
      const workbook = XLSX.read(result, { type: 'base64' });
      const sheet = workbook.Sheets['Ventas'];
      const json = XLSX.utils.sheet_to_json(sheet, { header: 1 }) as unknown[][];
    ...
    it('PR2 RED: hoja Arqueo muestra denominaciones y Total contado', async () => {
      const result = await service.generarExcelPalmar(record, resumenSemana);
      const workbook = XLSX.read(result, { type: 'base64' });
      const sheet = workbook.Sheets['Arqueo'];
      const json = XLSX.utils.sheet_to_json(sheet, { header: 1 }) as unknown[][];
    ...
    it('PR2 RED: hoja Resumen contiene totales semanales (recibido, efectivo, divisa, transferencia, invertido, ganancia)', async () => {
      const result = await service.generarExcelPalmar(record, resumenSemana);
      const workbook = XLSX.read(result, { type: 'base64' });
      const sheet = workbook.Sheets['Resumen'];
      const json = XLSX.utils.sheet_to_json(sheet, { header: 1 }) as unknown[][];
    ...
    it('PR2 RED (triangulación): Resumen con otra semana muestra otros totales', async () => {
      ...
      const result = await service.generarExcelPalmar(record, otraSemana);
      const workbook = XLSX.read(result, { type: 'base64' });
      const sheet = workbook.Sheets['Resumen'];
      const json = XLSX.utils.sheet_to_json(sheet, { header: 1 }) as unknown[][];
