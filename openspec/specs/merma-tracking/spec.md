...
The system MUST allow registering merma (product damage/loss) from the InventarioPage, consuming stock via FIFO.

#### Scenario: Register merma for a product

- GIVEN a product with stock > 0 in InventarioPage
- WHEN the user clicks "Merma", enters quantity and optional reason
- THEN stock MUST be reduced via FIFO (oldest lots consumed first)
- AND a `stock_movimientos` record with `tipo = 'merma'` is created
- AND `costo_total` on the movement equals the sum of FIFO consumption costs
...
The system MUST display merma entries in the JornadaPage daily table alongside sales and movements.

#### Scenario: Daily table includes merma section

- GIVEN a jornada with 2 merma entries
- WHEN the JornadaPage renders
- THEN a "Mermas" section MUST appear in the daily table
- AND each merma entry shows: product name, quantity, cost, timestamp
