...
Current: no backups exist; a lost/evicted OPFS database means accepted data loss.

Changed: automatic backups via `db.backup()` — rodante file at `Documents\Tienda - App\DataBase\tienda-app.db` on app open, jornada close, and app close; timestamped file per jornada close at `Documents\Tienda - App\DataBase\backups\tienda_<YYYY-MM-DD_HHmm>.db` with retention of 30 and auto-prune; ADOPT on fresh install; one-click manual export (desktop). All automatic — no user/admin interaction with the DB; UI feedback only on restore/adoption or blocking failure.

## ADDED Requirements

### Requirement: R1 — Automatic rodante backup
...
### Requirement: R2 — Timestamped backup at jornada close
...
### Requirement: R3 — Retention and auto-prune
...
### Requirement: R4 — ADOPT valid backup on fresh install
...
### Requirement: R5 — Manual export (desktop)
...
### Requirement: R6 — Automatic and non-fatal backups
