# Delta for database-storage

## Changed behavior vs current

Current: all platforms store via SQLocal on OPFS; on OPFS failure SQLocal silently falls back to `:memory:` (console.warn only); no `navigator.storage.persist()`; migrations v1–v16 live inside SqliteService.

Changed: desktop (Electron) stores in a native SQLite file at `app.getPath('userData')/tienda-app.db` via a new IPC-backed `Database` implementation (better-sqlite3 in main); initialization fails loud (blocking UI) instead of silently degrading; web/Capacitor keeps SQLocal but requests `navigator.storage.persist()`; migrations move to a shared runner. All 8 consumers keep identical SQL — no consumer spec changes.

## ADDED Requirements

### Requirement: R1 — Fail-loud database initialization
...
### Requirement: R2 — Runtime driver selection
...
### Requirement: R3 — Identical SQL for all consumers
...
### Requirement: R4 — Shared migration runner
...
### Requirement: R5 — Native database location
...
### Requirement: R6 — Single-statement SQL semantics
...
### Requirement: R7 — Async IPC only for SQL
...
### Requirement: R8 — One-shot OPFS→native import
...
### Requirement: R9 — Storage persistence on web
...
### Requirement: R10 — Version bump (delivery note)
...
### Requirement: R11 — Packaging includes main-process DB module (build requirement)
