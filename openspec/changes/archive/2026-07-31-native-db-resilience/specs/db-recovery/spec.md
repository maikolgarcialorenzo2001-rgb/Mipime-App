...
Current: no recovery exists — an OPFS open failure silently falls back to `:memory:` and data is lost without any feedback.

Changed: a startup recovery cascade (open working DB → in-place `.recover` → newest valid backup, rodante first then timestamped newest→oldest, validating each candidate with `integrity_check` + `schema_version`) restores data automatically; total failure produces blocking feedback "contactar al desarrollador" with diagnostics; every restore shows visual feedback (what restored, from when, what is lost). Fail-loud on any DB open/initialize failure — never silent `:memory:`.

## ADDED Requirements

### Requirement: R1 — Startup recovery cascade
...
### Requirement: R2 — Candidate validation
...
### Requirement: R3 — Blocking feedback when all recovery fails
...
### Requirement: R4 — Never silent restore
...
### Requirement: R5 — Fail-loud on open/initialize failure
