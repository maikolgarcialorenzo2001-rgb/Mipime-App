...
### T8 — Backup subsystem
- [x] T8 DONE (slice 4, 2026-07-31): (a) NEW `src/app/services/backup.service.ts` + spec (5 tests) — `backup('open'|'jornada-close')` → `invoke('db:backupNow',{trigger})`; web = no-op (AD-6); rechazo o `{ok:false}` → resuelve sin lanzar (R6). (b) `jornada.service.ts` `_cerrarAsync`: `await backup('jornada-close')` en try/catch tras `_ejecutarCierre` (AD-7: el snapshot incluye el cierre; fallo no interrumpe; +4 tests, uno con vi.waitFor para probar que el backup es awaited). (c) main window `close` handler: `backupRodanteSync(dbPathFor(), rodantePathFor())` sync best-effort (AD-8). (d) NEW `backupRodanteSync(livePath, rodantePath)` en `electron/db.ts` (VACUUM INTO con escape de comillas + rm previo porque VACUUM INTO falla si el destino existe; guardas: mismo path / live inexistente; NUNCA lanza R6; +3 tests real-fs en db.spec). (e) `timestampedBackupName` movido a `electron/db.ts` como única fuente (4 dígitos, compatible con TIMESTAMPED_RE; main.ts lo importa; +2 tests anti-drift). Path helpers (dbPathFor/rodantePathFor/backupsDirFor) hoisteados a scope de módulo en main.ts para que el close handler los vea. (+fix review slice 4, 2026-07-31: `NativeSqliteService.initialize()` invoca `await backup('open')` en try/catch TRAS runMigrations — R1 backup-at-open ya no es dead code; +2 tests: backup llamado después de migraciones y R6 ante fallo; commit 8fae7d9.)
- **Files**: NEW `src/app/services/backup.service.ts` + spec; MOD `src/app/services/jornada.service.ts` + spec, `electron/main.ts`, `electron/db.ts` + spec
...
- [x] T9 DONE (slice 4, 2026-07-31): (a) Coordination note RESUELTO: `electron/main.ts` db:import null-branch ahora captura el resultado de `adoptOrFresh` y devuelve `{ok:true, restoreInfo}` cuando `status==='adopted'` (fresh → `{ok:true}` a secas); `electron/types.d.ts` `DbImportResult` += `restoreInfo?: DbRestoreInfo` (+1 test main.spec). (b) `native-sqlite.service.ts` publica restoreInfo en DbStatusService SOLO cuando el resultado lo reporta (init restored/adopted, import adopt, re-init restored) — un status posterior 'ok' sin restoreInfo NUNCA borra un aviso ya publicado (el re-init del roundtrip mataría el aviso de adopt si se sobreescribiera con null; +4 tests). (c) NEW `src/app/components/restore-feedback/` (ts/html/css/spec, 10 tests) — toast transitorio NO bloqueante (fixed bottom-4 right-4 z-40, SIN overlay inset-0; distinto de db-error full-screen R4): título "Base de datos restaurada", origen (etiqueta por RestoreFrom), cuándo (toLocaleString), ventana de pérdida ("N min"/"N h", omitida si 0), botón descartar → setRestoreInfo(null). (d) `app.ts`/`app.html`: `<app-restore-feedback/>` renderiza `@if (dbStatus.restoreInfo())` FUERA del if/else bloqueante (coexiste con router-outlet, R4).
- **Files**: NEW `src/app/components/restore-feedback/restore-feedback.component.ts` + `.html` + `.css` + spec; MOD `src/app/app.ts`, `src/app/app.html`, `electron/main.ts`, `electron/types.d.ts`, `src/app/services/native-sqlite.service.ts` + spec; tests en `electron/main.spec.ts`
...
- [x] T10 DONE (slice 4, 2026-07-31): la cascada (working → recoverInPlace → rodante → timestamped newest→oldest → fatal+diagnostics con backupsTried) YA estaba implementada y testeada en slice 2 (db.spec: runStartupSequence cascade, corrupt repair, backupsTried con motivos por candidato). Slice 4 completó lo que faltaba: (a) wiring restoreInfo → DbStatusService → restore-feedback (R4, ver T9); (b) test R5 NUEVO en main.spec: db:sql abre la DB nativa en ruta real (`openNativeDb` llamado con `DB_FILE`, nunca ':memory:'). Sin cambios en db.ts para la cascada (ya cubierta).
- **Files**: tests en `electron/db.spec.ts`, `native-sqlite.service.spec.ts`; MOD `electron/db.ts`, `native-sqlite.service.ts` as needed
