...
└── app/
    ...
    │
    ├── models/                   # Interfaces de datos (incl. palmar-ventas)
    │
    ...
    │   ├── excel.service.ts      # Generación de XLSX (incl. Excel semanal Palmar)
    │   ├── backup.service.ts     # Backups rodantes + export
    │   ├── palmar.service.ts     # 🎯 Palmar: historial, detalle, reimprimir, semanal
    ...
    │   │                         # arqueo-billetes-form (compartido),
    ...
    └── pages/
        ...
        └── palmar/               # 🎯 Palmar: registrar jornada de la tienda externa
```
...
| `/palmar` | Palmar 🎯 | Admin | Registrar jornadas de la tienda externa Palmar (peculiaridad de esta rama) |
...
| Registrar jornadas de Palmar (`/palmar`) | ✅ | ❌ |
...
- **Palmar 🎯**: registro de jornadas de la tienda externa (0 escrituras en DB),
  Excel de 3 hojas + JSON, historial, reimpresión y resumen semanal.
...
> Las jornadas de Palmar **no** viven en estas tablas: se guardan como JSON +
> Excel en `Documents/Tienda - App/Palmar/` (ver peculiaridad arriba).
...
```bash
npm run version:bump   # 0.1.17-beta → 0.1.18-beta (preserva sufijo)
```

> **Nota de rama**: `palmar-feature` mantiene su propia versión (actualmente
> `0.1.17-beta`), puede ir adelantada o desfasada respecto de `main`.
...
palmar-feature        → 🎯 peculiaridad del cliente Palmar (NO mergea a main)
feature/<nombre>      → features (PRs incrementales hacia main)
fix/<nombre>          → hotfixes / correcciones
```

Los PRs apuntan a `main`, **excepto los de Palmar**, que apuntan a
`palmar-feature` (cadena de PRs propia, desacoplada de main).
...
| [docs/palmar-ventas-plan-prs.md](docs/palmar-ventas-plan-prs.md) | 🎯 Plan y reglas de negocio de Palmar |
